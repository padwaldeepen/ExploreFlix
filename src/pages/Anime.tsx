...
import { getAnime } from '../api/exploreflixApi';
...
export const Anime = () => {
  ...
  useEffect(() => {
    filterAndSortAnime();
  }, [anime, searchQuery, selectedGenre, selectedYear, sortBy]);
  ...
  const getAvailableYears = () => {
    const years = new Set(anime.map(item => item.year).filter(Boolean));
  ...
  if (error) {
    return (
      <div className={styles.errorContainer}>
        <h3>Error loading anime</h3>
        <p>{error}</p>
        <button onClick={() => window.location.reload()}>Try Again</button>
        <p style={{marginTop: '1rem', color: '#aaa'}}>If this keeps happening, our anime API may be down. Please try again later or check your connection.</p>
      </div>
    );
  }
        ...
        >
          <div className={styles.headerContent}>
            <h1 className={styles.pageTitle}>
              <span className={styles.titleAccent}>Anime</span> Collection
            </h1>
            <p className={styles.pageSubtitle}>
              Discover the best anime from around the world
            </p>
          </div>
        </motion.div>
